                ...
                borderColor:(UIColor *)borderColor
{
    ...
    const CGFloat imageScale = MAX(size.width / imageSize.width, size.height / imageSize.height);
    ...
    if (borderColor && borderWidth > 0) {
        const CGRect borderRect = ({
            const CGFloat x = borderWidth / 2;
            CGRectMake(x, x, size.width - 2 * x, size.height - 2 * x);
        });
        const CGSize cornerRadii = ({
            const CGFloat x = borderWidth / 2;
            CGSizeMake(cornerRadius - x, cornerRadius - x);
        });
    ...
    const CGRect drawRect = ({
        CGRect rect = CGRectMake(0, 0, imageSize.width * imageScale, imageSize.height * imageScale);
