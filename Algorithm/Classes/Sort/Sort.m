...
{
    NSString *s0 = @"a";
    NSString *s1 = [s0 mutableCopy];
    NSString *s2 = [s1 copy];
    NSLog(@"%p %p %p", s0, s1, s2);
    NSLog(@"%p %p %p", &s0, &s1, &s2);
    NSLog(@"%@ %@ %@", object_getClass(s0), object_getClass(s1), object_getClass(s2));
    
    __block NSInteger index = 0;
    void (^try)(SEL selector, NSArray *) = ^(SEL selector, NSArray *array) {
        NSArray *result = [self bubbleSort:array];
    ...
    try(@selector(bubbleSort:), array);
    try(@selector(insertionSort:), array);
    try(@selector(selectSort:), array);
    
    try(@selector(quickSort:), array);
    try(@selector(shellSort:), array);
    try(@selector(mergeSort:), array);
    try(@selector(heapSort:), array);
    
    try(@selector(countingSort:), array);
    try(@selector(bucketSort:), array);
    try(@selector(radixSort:), array);
}
...
void heap_sort(NSMutableArray *tree, NSInteger n) {
    // 先把一个数组组成一个堆
    bulid_heap(tree, n);
    // 从最后一个节点开始
...
void bulid_heap(NSMutableArray *tree, NSInteger n) {
    NSInteger last_node = n - 1;
    // 节点i的父节点的索引i= (i - 1) / 2
    NSInteger parent = (last_node - 1) / 2;
    // 从最后一个父节点进行堆重组
...
void swap(NSMutableArray *tree, NSInteger max, NSInteger i) {
    NSObject *temp = tree[i];
    tree[i] = tree[max];
    tree[max] = temp;
}

// 对第i个节点进行堆重组，n为数组tree元素的个数
void heapify(NSMutableArray *tree, NSInteger n, NSInteger i) {
    // 递归出口
    if (i >= n) {
        return;
    }
